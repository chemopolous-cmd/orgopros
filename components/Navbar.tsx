...
export default function Navbar() {
  ...
  // Close on Escape and trap focus inside the mobile panel when open
  useEffect(() => {
  ...
  // When opening, move focus to first focusable link
  useEffect(() => {
    if (!open) return;
    const panel = panelRef.current;
    if (!panel) return;
  ...
  return (
    ...
    >
      <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4 lg:px-8">
        ...
        <Link
          href="/"
          className="text-xl font-bold text-brand-teal hover:text-brand-tealHover"
          onClick={() => setOpen(false)}
        >
          Orgopros
        </Link>

        {/* Desktop nav (no Get Started link here) */}
        ...
        <div className="hidden md:flex items-center gap-4">
          {/* Force solid TEAL background (with !important) */}
          <Link
            href="/get-started"
            className="rounded-xl !bg-brand-teal px-4 py-2 text-sm font-semibold !text-white hover:!bg-brand-tealHover"
          >
            Get Started
          </Link>
          {/* Outlined teal → Become a Tutor */}
          <Link
            href="/tutor-signup"
            className="rounded-xl border border-brand-teal px-4 py-2 text-sm font-semibold text-brand-teal hover:bg-brand-navy"
          >
            Become a Tutor
          </Link>
        </div>
        ...
        <div className="mx-auto max-w-7xl px-6">
          <div className="rounded-2xl border border-brand-navy/30 bg-brand-dark/95 backdrop-blur p-6">
            ...
            <div className="mt-6 grid gap-3">
              {/* Force solid TEAL on mobile CTA (with !important) */}
              <Link
                href="/get-started"
                onClick={() => setOpen(false)}
